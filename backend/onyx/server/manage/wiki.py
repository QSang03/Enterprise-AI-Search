...
def parse_and_enforce_citations(markdown_content: str, valid_chunk_ids: Set[uuid.UUID]) -> tuple[str, List[tuple[str, uuid.UUID]]]:
    """
...
def generate_wiki(request: GenerateWikiRequest, db_session: Session = Depends(get_session), _: User = Depends(current_user)) -> WikiPageResponse:
    ...
    # Get all chunks for these document IDs
    chunks = db_session.query(DocumentChunkV2).filter(
        DocumentChunkV2.doc_id.in_(request.document_ids)
    ).all()
    ...
    prompt = f"""Bạn là biên tập viên kỹ thuật cao cấp chuyên nghiệp.
Nhiệm vụ của bạn là tổng hợp một tài liệu Wiki markdown có cấu trúc phân cấp (sử dụng #, ##, ###) từ các phân mảnh tài liệu nguồn sau.

Yêu cầu BẮT BUỘC:
1. Viết bằng tiếng Việt sáng rõ, logic, mạch lạc.
2. Mỗi đề mục (section), ý chính hoặc thông tin trích dẫn phải kết thúc bằng thẻ trích dẫn chính xác định dạng `[Citation: <chunk_uuid>]`.
3. Chỉ trích dẫn đúng UUID của phân mảnh cung cấp thông tin đó (UUID có trong danh sách phân mảnh nguồn bên dưới).
...
"""
    ...
    # Enforce citations
    final_content, citations = parse_and_enforce_citations(generated_text, valid_chunk_ids)
    
    if not final_content.strip():
        raise OnyxError(
            OnyxErrorCode.BAD_REQUEST,
            "Failed to publish Wiki because no section contained valid citations to the source documents."
        )
        
    # Check if page with same title exists
    existing = db_session.query(WikiPage).filter(
        WikiPage.title == title
    ).order_by(desc(WikiPage.version)).first()
...
@router.get("")
def list_wikis(db_session: Session = Depends(get_session), _: User = Depends(current_user)) -> List[WikiPageResponse]:
    # Retrieve latest version for each wiki_id
    subquery = db_session.query(
        WikiPage.wiki_id,
        func.max(WikiPage.version).label("max_version")
    ).group_by(WikiPage.wiki_id).subquery()
    
    latest_pages = db_session.query(WikiPage).join(
        subquery,
        (WikiPage.wiki_id == subquery.c.wiki_id) & (WikiPage.version == subquery.c.max_version)
    ).all()
...
@router.get("/{wiki_id}")
def get_wiki(wiki_id: str, version: Optional[int] = None, db_session: Session = Depends(get_session), _: User = Depends(current_user)) -> WikiPageResponse:
    try:
...
def list_stale_events(db_session: Session = Depends(get_session), _: User = Depends(current_user)) -> List[StaleEventResponse]:
    events = db_session.query(WikiStaleEvent).filter(
        WikiStaleEvent.resolved == False
    ).all()
    return [
        StaleEventResponse(
...
def resolve_stale_event(event_id: str, db_session: Session = Depends(get_session), _: User = Depends(current_user)) -> dict:
    try:
    ...
    event = db_session.query(WikiStaleEvent).filter(
        WikiStaleEvent.event_id == e_uuid
    ).first()
