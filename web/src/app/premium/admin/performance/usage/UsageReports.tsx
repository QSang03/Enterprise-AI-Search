...
function GenerateReportInput({
  ...
  const requestReport = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw Error(`${t("usageReports.receivedError")}: ${res.statusText}`);
      }
  ...
  return (
    <div className="mb-8">
      <Title className="mb-2">{t("usageReports.title")}</Title>
      <Text as="p">{t("usageReports.description")}</Text>
      <Spacer rem={2} />
      ...
      >
        {isWaitingForReport ? t("usageReports.generating") : t("usageReports.generateReport")}
      </OpalButton>
      <p className="mt-1 text-xs">
        {isWaitingForReport
          ? t("usageReports.reportGenerating")
          : t("usageReports.reportBackgroundNote")}
      </p>
      {errorOccurred && (
        <ErrorCallout
...
function UsageReportsTable({
  ...
  return (
    <div>
      <Title className="mb-2 mt-6 mx-auto"> {t("usageReports.previousReports")} </Title>
            ...
            <TableBody>
              {paginatedReports.map((r) => (
                <TableRow key={r.report_name}>
                  ...
                  <TableCell>{r.requestor ?? t("usageReports.autoGenerated")}</TableCell>
                  <TableCell>
...
export default function UsageReports() {
  ...
  const handleReportGenerated = () => {
    ...
    timeoutRef.current = setTimeout(() => {
      ...
      setTimeoutMessage(
        t("usageReports.reportGenerationSlow")
      );
              ...
              <div className="flex-1">
                <div className="text-status-warning-05">
                  <Text as="p" font="main-ui-action">
                    {t("usageReports.reportInProgress")}
                  </Text>
                </div>
                <Spacer rem={0.25} />
                <div className="text-status-warning-05">
